...
temp_root=$(mktemp -d "${RUNNER_TEMP:-/tmp}/loqculate-wiki.XXXXXX")
...
if ! git -c "http.extraheader=AUTHORIZATION: basic ${auth_header}" \
  ...
  echo "could not clone the GitHub Wiki; it may be disabled or empty" >&2
  exit 1
fi

wiki_branch=$(git -C "$wiki_repo" symbolic-ref --quiet --short HEAD) || {
  echo "GitHub Wiki clone has no default branch" >&2
  exit 1
}
