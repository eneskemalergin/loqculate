"""Regression tests: v2 PiecewiseWLS must reproduce v1 LOD/LOQ on reference data.

Strategy
--------
We cannot import v1 directly (it uses lmfit which may not be installed).  Instead
we hard-code reference output generated by running v1 on the ``one_protein.csv``
demo dataset.  Since we use the same initialization strategy (legacy) and the same
mathematics, the results must match to within floating-point tolerance.

If you need to regenerate the reference values::

    cd v1 && python calculate-loq.py \
