...
# Tolerance for regression check: 1e-6 (deterministic on a platform; cross-BLAS headroom).
_PINNED_LOD = {
...
class TestLODRegression:
    def test_lod_matches_pinned_values(self, all_cf_fits):
        """PiecewiseCF.lod() must match the pinned reference within 1e-6.

        The solver is deterministic on a given platform; 1e-6 absorbs
        cross-platform float noise without hiding real knot/LOD regressions.
        """
        failures = []
...
class TestEdgeCases:
    ...
    def test_lod_invariant_to_sort_order(self, reference_data):
        ...
        # different order → a few ULPs.  rtol=1e-8 distinguishes cross-platform
        # noise from a different knot (which would differ by multiple percent).
        np.testing.assert_allclose(
            lod_original,
            lod_shuffled,
            rtol=1e-8,
...
class TestVectorizedBootstrap:
    ...
    - ADTGIAVEGATDAAR in particular agrees tightly for seed=42
      (within 1e-8 across platforms)
    """
    ...
    # Loop vs vectorized bootstrap: hold to 1e-8 across platforms (BLAS order).
    def test_h6_cv_profile_agrees_with_loop(self, pep_data, x_grid):
        """ADTGIAVEGATDAAR seed=42: CV profiles agree within 1e-8."""
        x, y, W = pep_data
        n_reps, seed = 100, 42
        ...
        assert max_diff < 1e-8, f"ADTGIAVEGATDAAR: max |cv_loop - cv_vec| = {max_diff:.3e}"

    def test_h6_mean_pred_agrees_with_loop(self, pep_data, x_grid):
        """Mean predictions agree within 1e-8 for ADTGIAVEGATDAAR seed=42."""
        x, y, W = pep_data
        n_reps, seed = 100, 42
        ...
        assert max_diff < 1e-8, f"ADTGIAVEGATDAAR: max |mean_loop - mean_vec| = {max_diff:.3e}"
    ...
    def test_h6_all_peptides_agree_with_loop(self, reference_data):
        ...
        Tolerance: max|cv_diff| < 1e-5 and max|mean_diff|/mean_pred < 1e-5.
        Tight enough to catch knot-selection disagreement; loose enough for
        cross-platform reduction order differences.
        """
        n_reps, seed = 200, 42
        failures = []
        for pep in np.unique(reference_data.peptide):
            ...
            if d_cv >= 1e-5 or d_mean_rel >= 1e-5:
