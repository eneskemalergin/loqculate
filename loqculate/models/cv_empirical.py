...
def _vectorized_cv_stats(
    x: np.ndarray, y: np.ndarray,
) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """Compute per-group count, mean, std (ddof=1), CV using bincount.

    Returns (unique_concs_sorted, counts, means, cvs).
    CVs are NaN where count < 2 or mean == 0.
    """
    ...
    # Bessel-corrected variance: var = (Σ(xi²)/n - mean²) * n/(n-1)
    sum_sq = np.bincount(inverse, weights=y * y, minlength=n_groups)
    with np.errstate(invalid='ignore', divide='ignore'):
        var = (sum_sq / counts - means ** 2) * counts / (counts - 1.0)
    var = np.maximum(var, 0.0)  # guard floating-point negative zeros
    stds = np.sqrt(var)

    with np.errstate(invalid='ignore', divide='ignore'):
        cvs = np.where(means != 0, stds / means, np.nan)

    # NaN out groups with < 2 replicates (std undefined)
    cvs[counts < 2] = np.nan
