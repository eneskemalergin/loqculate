...
def plot_calibration(
    ...
    ax_top.plot(x_line, model.predict(x_line), color="green", linewidth=1.4, label="fit")

    # Bootstrap CI band (if the model supports it)
    try:
        lower, upper = model.prediction_interval(x_line)
        ax_top.fill_between(x_line, lower, upper, color="gold", alpha=0.3, label="95 % PI")
    ...
    ax_bot.axhline(
        cv_thresh, color="red", linestyle="--", linewidth=1.0, label=f"{cv_thresh:.0%} CV"
    )
    if np.isfinite(lod):
