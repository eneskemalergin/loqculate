...
def plot_model_comparison(
    ...
    ax_bot.axhline(
        cv_thresh, color="black", linestyle="--", linewidth=0.8, label=f"{cv_thresh:.0%} CV"
    )
    ...
    for idx, (label, model) in enumerate(models_dict.items()):
        ...
        if np.isfinite(lod):
            ax_top.axvline(lod, color=colour, linestyle=":", linewidth=1.2, label=lod_label)
            ax_bot.axvline(lod, color=colour, linestyle=":", linewidth=1.0)
        if np.isfinite(loq):
            ax_top.axvline(loq, color=colour, linestyle="--", linewidth=1.2, label=loq_label)
            ax_bot.axvline(loq, color=colour, linestyle="--", linewidth=1.0)

        # CV profile
        if (
            hasattr(model, "_x_grid")
            and model._x_grid is not None
            and model._boot_summary is not None
        ):
            ax_bot.plot(
                model._x_grid,
                model._boot_summary["cv"],
                color=colour,
                linewidth=1.2,
                label=f"{label} CV",
            )
